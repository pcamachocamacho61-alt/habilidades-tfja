        ...
        <div className="space-y-6">
          {blocks.map((block) => {
            ...
            return <div key={block}><h3 className="text-xs font-bold uppercase tracking-[0.2em] text-slate-400">Bloque {block}</h3><div className="mt-3 space-y-2">{blockSteps.map((step) => {
              ...
              return <Link key={step.id} href={`${basePath}/${step.id}`} onClick={() => setMobileMenuOpen(false)} className={active ? "block rounded-2xl border border-blue-200 bg-blue-50 p-3" : "block rounded-2xl border border-slate-100 bg-slate-50 p-3 hover:bg-white"} aria-current={active ? "step" : undefined}><div className="flex gap-3"><span className={completed ? "mt-1 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#0b376d] text-[10px] font-bold text-white" : step.type === "evaluation" ? "mt-1 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#c78b3a] text-[10px] font-bold text-white" : "mt-1 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-slate-200 text-[10px] font-bold text-slate-500"}>{completed ? "✓" : step.type === "evaluation" ? "★" : step.number}</span><div><p className="text-sm font-bold text-[#061b3a]">{title(step)}</p><p className="mt-1 text-xs text-slate-500">{label(step)}</p></div></div></Link>;
            })}</div></div>;
          })}
        </div>
      </div>
