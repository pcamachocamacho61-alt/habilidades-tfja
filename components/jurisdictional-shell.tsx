...
type JurisdictionalShellProps = {
  children: ReactNode;
};

export function JurisdictionalShell({
  children,
}: JurisdictionalShellProps) {
  const pathname = usePathname();
  const [sidebarOpen, setSidebarOpen] = useState(true);
  const [isAdmin, setIsAdmin] = useState(false);

  const isContentRoute =
    /^\/sistemas-jurisdiccionales\/(consulta\/(sicsej|sjl2)|roles\/[^/]+\/[^/]+)/.test(
      pathname
    );
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "habilidades-tfja:session-changed",
      syncRole
    );
  ...
  const isHomeActive =
    pathname === "/sistemas-jurisdiccionales";
  ...
  function getMenuItemClass(active: boolean) {
    return active
      ? "flex min-h-[50px] items-center gap-4 rounded-2xl bg-blue-50 px-4 py-3.5 text-blue-700"
      : "flex min-h-[50px] items-center gap-4 rounded-2xl px-4 py-3.5 transition hover:bg-blue-50 hover:text-blue-700";
  }
  ...
  const menuContent = (
    <>
      <nav className="space-y-2 text-sm font-semibold text-slate-600">
        ...
        >
          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-2xl bg-[#fff8ef] text-[#c78b3a]">
            <BadgeIcon />
          </span>
        ...
        >
          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-2xl bg-blue-50 text-[#0b376d]">
            <AlertIcon />
          </span>
          <span>Alertas</span>
          <SidebarAlertCount />
        ...
        >
          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-2xl bg-slate-100 text-[#061b3a]">
            <ProfileIcon />
          </span>
          <span>Mi perfil</span>
        </Link>
      ...
      <div className="mt-auto border-t border-slate-200 pt-6">
        <div className="flex w-full flex-col items-center justify-center text-center">
          <Image
            ...
            width={72}
            height={72}
            className="mx-auto block w-[72px]"
            style={{ height: "auto" }}
          />

          <p className="mt-5 text-center text-xs font-black uppercase leading-5 tracking-[0.26em] text-[#061b3a]">
            Menos clics,
            <br />
            <span className="text-[#c78b3a]">
              más justicia.
            </span>
          </p>
        </div>
  ...
  return (
    <main className="min-h-screen bg-[#eef7fd]">
        ...
        <div className="flex w-full items-center justify-between gap-3 px-4 py-3 sm:px-5 xl:px-8 2xl:px-12">
          <div className="flex min-w-0 items-center gap-3 sm:gap-4">
            <button
              ...
              aria-controls={
                isContentRoute
                  ? "jurisdictional-content-menu"
                  : "jurisdictional-sidebar jurisdictional-mobile-menu"
              }
            >
              ☰
            </button>
            ...
            <Link
              ...
              aria-label="Ir al inicio de Habilidades TFJA"
            >
              <Image
            ...
            <div className="hidden md:block">
              ...
              <p className="text-xs text-slate-500">
                Inicio &gt; Sistemas y aplicaciones
                jurisdiccionales
              </p>
            </div>
          </div>
      ...
      <DemoAuthGuard>
        {isContentRoute ? (
          ...
          <section
            className={
              sidebarOpen
                ? "grid w-full gap-6 py-0 pr-4 pl-0 sm:pr-6 lg:grid-cols-[260px_minmax(0,1fr)] xl:pr-10 2xl:pr-14"
                : "grid w-full gap-5 px-4 py-5 sm:px-5 sm:py-6 lg:grid-cols-[minmax(0,1fr)] xl:px-8 2xl:px-14"
            }
          >
            {sidebarOpen && (
              <aside
                id="jurisdictional-sidebar"
                className="sticky top-[82px] hidden h-[calc(100dvh-82px)] min-h-0 self-start overflow-y-auto overscroll-contain rounded-r-[24px] border border-l-0 border-white bg-white/90 p-5 shadow-[0_18px_55px_rgba(15,23,42,0.08)] lg:flex lg:flex-col"
                aria-label="Menú de sistemas jurisdiccionales"
              >
                {menuContent}
              </aside>
            )}
      ...
      {isContentRoute && (
        <aside
          id="jurisdictional-content-menu"
          className={
            sidebarOpen
              ? "fixed inset-y-0 left-0 z-[100] flex w-[86%] max-w-[320px] translate-x-0 flex-col overflow-y-auto overscroll-contain bg-white p-5 shadow-[18px_0_55px_rgba(15,23,42,0.18)] transition-transform duration-300 ease-out sm:w-[320px]"
              : "pointer-events-none fixed inset-y-0 left-0 z-[100] flex w-[86%] max-w-[320px] -translate-x-full flex-col overflow-y-auto overscroll-contain bg-white p-5 shadow-[18px_0_55px_rgba(15,23,42,0.18)] transition-transform duration-300 ease-out sm:w-[320px]"
          }
          aria-label="Menú general de sistemas jurisdiccionales"
        ...
        >
          <div className="mb-5 flex items-center justify-between gap-4 border-b border-slate-200 pb-4">
            ...
            <button
              ...
              className="flex h-9 w-9 shrink-0 items-center justify-center rounded-2xl bg-slate-100 text-xl font-black text-[#061b3a] transition hover:bg-slate-200"
      ...
      {!isContentRoute && (
        <aside
          id="jurisdictional-mobile-menu"
          className={
            sidebarOpen
              ? "fixed inset-y-0 left-0 z-[100] flex w-[86%] max-w-[310px] translate-x-0 flex-col overflow-y-auto bg-white p-5 shadow-[18px_0_55px_rgba(15,23,42,0.18)] transition-transform duration-300 lg:hidden"
              : "pointer-events-none fixed inset-y-0 left-0 z-[100] flex w-[86%] max-w-[310px] -translate-x-full flex-col overflow-y-auto bg-white p-5 shadow-[18px_0_55px_rgba(15,23,42,0.18)] transition-transform duration-300 lg:hidden"
          }
          aria-label="Menú de sistemas jurisdiccionales"
          aria-hidden={!sidebarOpen}
        >
